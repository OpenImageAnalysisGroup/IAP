 ...
 * <p>
 * ImageJ is a work of the United States Government. It is in the public domain and open source. There is no copyright. You are free to do anything you want
 * with this source but I like to get credit for my work and I would like you to offer your changes to me so I can possibly add them to the "official" version.
 * 
 * <pre>
			...
			switch (keyCode) {
				...
				case KeyEvent.VK_DOWN: // arrow keys
					...
					if (stackKey && keyCode == KeyEvent.VK_RIGHT)
						...
						if (stackKey && keyCode == KeyEvent.VK_LEFT)
							cmd = "Previous Slice [<]";
						else
							if (zoomKey && keyCode == KeyEvent.VK_DOWN && !ignoreArrowKeys(imp))
								cmd = "Out";
							else
								if (zoomKey && keyCode == KeyEvent.VK_UP && !ignoreArrowKeys(imp))
									cmd = "In";
								else
									if (roi != null) {
			...
			if (arg.startsWith("-")) {
				if ((arg.startsWith("-macro") || arg.startsWith("-batch")) && i + 1 < nArgs) {
					...
					if (arg.startsWith("-eval") && i + 1 < nArgs) {
						...
						if (arg.startsWith("-run") && i + 1 < nArgs) {
							IJ.run(args[i + 1]);
							args[i + 1] = null;
						}
			} else
				if (macros == 0 && (arg.endsWith(".ijm") || arg.endsWith(".txt"))) {
					...
					if (arg.length() > 0 && arg.indexOf("ij.ImageJ") == -1) {
					...
					if (arg.startsWith("-macro") && i + 1 < nArgs) {
						...
						if (arg.startsWith("-eval") && i + 1 < nArgs) {
							...
							if (arg.startsWith("-run") && i + 1 < nArgs) {
								...
								if (arg.indexOf("ij.ImageJ") == -1 && arg.length() > 0 && !arg.startsWith("-"))
									cmd = "open " + arg;
				if (cmd != null) {
					sendArgument(cmd);
					nCommands++;
