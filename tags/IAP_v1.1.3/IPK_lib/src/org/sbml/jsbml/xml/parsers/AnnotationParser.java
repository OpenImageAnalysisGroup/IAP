/*
 * $Id: AnnotationParser.java,v 1.1 2012-11-07 14:43:35 klukas Exp $
 * $URL: https://jsbml.svn.sourceforge.net/svnroot/jsbml/trunk/core/src/org/sbml/jsbml/xml/parsers/AnnotationParser.java $
 * ----------------------------------------------------------------------------
...
public class AnnotationParser implements ReadingParser {
			...
			Object contextObject) 
	{
		...
		logger.error("processNamespace : elementName = " + elementName);
		logger.error("processNamespace : namespace uri = " + URI);
		logger.error("processNamespace : namespace prefix = " + prefix);
		logger.error("processNamespace : namespace localName = " + localName);				
	...
	{
		logger.error(String.format("processing startElement %s. %s", elementName, prefix));
		...
		if (contextObject instanceof Annotation) {
			...
			// There is a syntax error, the node can't be read ?
			logger.error(String.format("Cannot read the element %s as the context object is not of the type Annotationblahblah", elementName)); 
		}
