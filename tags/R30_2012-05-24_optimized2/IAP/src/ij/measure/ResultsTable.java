...
public class ResultsTable implements Cloneable {
	 ...
	 * After labels are added or modified, call <code>show()</code> to update the window displaying the table.
	 */
	 ...
	 * not exist, it is created. When adding columns, <code>show()</code> must be called to update the
	...
	public void update(int measurements, ImagePlus imp, Roi roi) {
		...
		for (int i = 0; i <= last; i++) {
			...
			if (rt2.getColumn(i) != null && columns[i] == null) {
				...
				if (rt2.getColumn(i) == null && columns[i] != null && !keep[i])
					columns[i] = null;
		}
		if (rt2.getRowLabels() == null)
			rowLabels = null;
		else
			if (rt2.getRowLabels() != null && rowLabels == null) {
				rowLabels = new String[maxRows];
				rowLabelHeading = "Label";
			}
		if (getCounter() > 0)
