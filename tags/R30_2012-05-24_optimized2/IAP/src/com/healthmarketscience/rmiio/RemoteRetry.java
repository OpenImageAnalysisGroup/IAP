 ...
 * <p>
 * The major caveat for this class is the remote method call <b>must be idempotent</b>. This means, in essence, that repeated calls with the same arguments
 * should generate the exact same results. An example of a non-idempotent call would be "remove $10 from my bank account". If this call were sent twice (because
 * the first attempt seemed to fail), you could end up with $20 removed from your bank account. In order to solve this, you could add a unique sequence id to
 * the call, "remove $10 from my bank account, seqId(5)", and the server could ignore the second call (assuming it was keeping track of the sequence ids that it
 * had processed thus far). In other words, remote method calls are a lot harder to make than local method calls and much care should be taken when dealing with
 * remote APIs.
 * <p>
 * Although RemoteRetry is an abstract class, there are a variety of simple implementations, as well as static instances of these implementations. See the
...
public abstract class RemoteRetry {
	 ...
	 * <li>Retries 1 - 30 wait that many seconds each time before returning (after 1st retry wait 1 second, after 2nd retry wait 2 seconds...)
	 * <li>Retries > 30 wait 30 seconds each time before returning
	 * </ol>
	 * 
	 * @param numRetries
	 *           number of retries which have happended thus far
	 * @param log
	 *           debug log
	 */
	...
	private static Error handleNoMatches(
			Throwable throwable) {
		return (InternalError) (new InternalError("Impossible exception thrown"))
				.initCause(throwable);
	}
	 ...
	 * @return <code>true</code> iff call should be repeated, <code>true</code> otherwise
	 */
	 ...
	 * number of times and uses the backoff strategy from {@link RemoteRetry#simpleBackOff}.
	 */
	 ...
	 * strategy from {@link RemoteRetry#simpleBackOff}. Please read warning in {@link Always} before using.
	 */
