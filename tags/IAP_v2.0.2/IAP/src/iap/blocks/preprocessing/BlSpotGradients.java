 ...
 * @author klukas
 */
public class BlSpotGradients extends AbstractBlock {
	...
	public String getName() {
		return "Circular Gradient Matcher (L*a*b*)";
	}
	
	@Override
	public String getDescription() {
		return "Detect gradients in L/a/b color space. Spot size can be specified.";
	}
			...
			for (int x = 0; x < w; x++) {
				for (int y = 0; y < h; y++) {
					...
					if (color != ImageOperation.BACKGROUND_COLORint) {
						...
						statI.clear();
						statO.clear();
						for (int offX = 0; offX <= sr2; offX += sr) {
							for (int offY = 0; offY <= sr2; offY += sr) {
