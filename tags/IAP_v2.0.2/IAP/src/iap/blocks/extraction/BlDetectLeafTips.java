...
public class BlDetectLeafTips extends AbstractBlock implements CalculatesProperties {
	...
	protected Image processMask(Image mask) {
		...
		if (getBoolean("Calculate on " + ct.getNiceName() + " Image", ct == CameraType.VIS) && !ignore) {
			...
			savePeaksAndFeatures(
					getPeaksFromBorder(workimg, mask, searchRadius, (int) (searchRadius * getDouble("Factor for Width Estimation", 1.5)), fillGradeInPercent,
							mask.getCameraType()), ct,
	...
	private void savePeaksAndFeatures(GapList<Feature> peakList, CameraType cameraType, CameraPosition cameraPosition,
		...
		if (saveAdditionalFeatures) {
			...
			for (Feature bf : peakList) {
				...
				if (widthNearLT != null && widthNearLT > 0) {
		...
		if (saveFeaturesInResultSet || markResults) {
			...
			for (Feature bf : peakList) {
				...
				ArrayList<PositionAndColor> near_tip_area = (ArrayList<PositionAndColor>) bf.getFeature("pixelsNearTip");
				...
				if (searchRadius > 0) {
					...
					getResultSet().addImagePostProcessor(new RunnableOnImageSet() {
						...
						public Image postProcessMask(Image mask) {
							...
							double cornerA = cornerAngle / 360 * 2 * Math.PI / 2;
							Vector2d cAA = new Vector2d(d.getX(), d.getY()).rotate(cornerA);
							Vector2d cBB = new Vector2d(d.getX(), d.getY()).rotate(-cornerA);
							...
							if (near_tip_area != null && near_tip_area.size() > 0) {
								for (PositionAndColor pac : near_tip_area) {
									int x = pac.x;// - searchRadius / 2;
									int y = pac.y;// - searchRadius / 2;
									t = t.drawLine(x, y, x + 1, y, Color.ORANGE.getRGB(), 0.9d, 1);
								}
							}
							if (tip_area != null && tip_area.size() > 0) {
								for (PositionAndColor pac : tip_area)
									t = t.drawLine(pac.x, pac.y, pac.x + 1, pac.y, Color.CYAN.getRGB(), 0.8d, 1);
							}
							t = t.drawCircle((int) pos_fin.getX(), (int) pos_fin.getY(), searchRadius_fin, Color.RED.getRGB(), 0.8, 2)
									.drawLine(xPos, yPos, (int) cAA.getX() + xPos, (int) cAA.getY() + yPos, Color.ORANGE.getRGB(), 0.5, 1)
									.drawLine(xPos, yPos, (int) cBB.getX() + xPos, (int) cBB.getY() + yPos, Color.ORANGE.getRGB(), 0.5, 1)
									...
									.text((int) direction_fin.getX() + 10, (int) direction_fin.getY(),
											...
											Color.BLACK);
							// if (isSide) {
							String s = "L.Angle: " + angle.intValue() + " | C.Angle: " + (int) cornerAngle;
							s += " | Width near tip: " + (diff != null ? StringManipulationTools.formatNumber(diff, 1) : "n/a");
	...
	private GapList<Feature> getPeaksFromBorder(Image img, Image orig, int searchRadius, int searchRadius2, double fillGradeInPercent, CameraType ct) {
		...
		ba = new BorderAnalysis(img, orig, innerHull);
