...
public class VolumeLoader extends TemplateLoaderMMD {
	...
	public String[] getValidExtensions() {
		return new String[] { "hdr", "raw", "img" };// , "dcm", "tif" };
	}
	
	@Override
	public TemplateLoaderInterface createInstance(File f) {
		// if (f.getAbsolutePath().toLowerCase().endsWith(".dcm"))
		// return new DICOMVolumeLoaderInstance(f, this);
		// else
		// if (f.getAbsolutePath().toLowerCase().endsWith(".tif"))
		// return new TIFFVolumeLoaderInstance(f, this);
		// else
		return new VolumeLoaderInstance(f, this);
	}
