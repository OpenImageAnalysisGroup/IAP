...
{ /* begin class IODialog */
	...
	public void actionPerformed(final ActionEvent ae)
	{
		...
		if (ae.getActionCommand().equals("Save Landmarks As...")) {
			...
			if (ae.getActionCommand().equals("Load Landmarks...")) {
				loadPoints();
			}
			else
				if (ae.getActionCommand().equals("Show Landmarks")) {
					showPoints();
				}
				else
					if (ae.getActionCommand().equals("Load Elastic Transformation")) {
						loadTransformation();
					}
					else
						if (ae.getActionCommand().equals("Load Raw Transformation")) {
							loadRawTransformation();
						}
						else
							if (ae.getActionCommand().equals("Compare Opposite Elastic Transformations")) {
								compareOppositeElasticTransformations();
							}
							else
								if (ae.getActionCommand().equals("Compare Elastic/Raw Transformations")) {
									compareElasticWithRaw();
								}
								else
									if (ae.getActionCommand().equals("Compare Raw Transformations")) {
										compareRawTransformations();
									}
									else
										if (ae.getActionCommand().equals("Convert Transformation To Raw")) {
											saveTransformationInRaw();
										}
										else
											if (ae.getActionCommand().equals("Convert Transformation To Elastic")) {
												saveTransformationInElastic();
											}
											else
												if (ae.getActionCommand().equals("Compose Elastic Transformations")) {
													composeElasticTransformations();
												}
												else
													if (ae.getActionCommand().equals("Compose Raw Transformations")) {
														composeRawTransformations();
													}
													else
														if (ae.getActionCommand().equals("Compose Raw and Elastic Transformations")) {
															composeRawElasticTransformations();
														}
														else
															if (ae.getActionCommand().equals("Invert Raw Transformation")) {
																invertRawTransformation();
															}
															else
																if (ae.getActionCommand().equals("Evaluate Image Similarity")) {
																	evaluateSimilarity();
																}
																else
																	if (ae.getActionCommand().equals("Adapt Coefficients")) {
																		adaptCoefficients();
																	}
																	else
																		if (ae.getActionCommand().equals("Load Source Mask")) {
																			loadSourceMask();
																		}
																		else
																			if (ae.getActionCommand().equals("Load Source Initial Affine Matrix")) {
	...
	private void saveTransformationInElastic()
	{
		...
		String sInput = JOptionPane
				.showInputDialog(null, "Number of intervals for B-spline grid?", "Save as B-spline coefficients", JOptionPane.QUESTION_MESSAGE);
	...
	private void evaluateSimilarity()
	{
		...
		if (sourceIp instanceof ByteProcessor)
		{
			...
			if (sourceIp instanceof ShortProcessor)
			{
				final short[] pixels = (short[]) sourceIp.getPixels();
				for (int y = 0; (y < sourceHeight); y++)
					for (int x = 0; (x < sourceWidth); x++, k++)
						if (pixels[k] < (short) 0)
							sourceImage[k] = (double) pixels[k] + 65536.0F;
						else
							sourceImage[k] = pixels[k];
			}
			else
				if (sourceIp instanceof FloatProcessor)
				{
					...
					if (sourceIp instanceof ColorProcessor)
					{
		...
		if (targetIp instanceof ByteProcessor)
		{
			...
			for (int y = 0; (y < targetHeight); y++)
				for (int x = 0; (x < targetWidth); x++, k++)
					targetImage[k] = pixels[k] & 0xFF;
		}
		else
			if (targetIp instanceof ShortProcessor)
			{
				final short[] pixels = (short[]) targetIp.getPixels();
				for (int y = 0; (y < targetHeight); y++)
					for (int x = 0; (x < targetWidth); x++, k++)
						if (pixels[k] < (short) 0)
							targetImage[k] = (double) pixels[k] + 65536.0F;
						else
							targetImage[k] = pixels[k];
			}
			else
				if (targetIp instanceof FloatProcessor)
				{
					...
					if (targetIp instanceof ColorProcessor)
					{
