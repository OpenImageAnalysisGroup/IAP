...
public class TabStatistics extends InspectorTab implements ActionListener, ContainsTabbedPane {
	...
	private boolean calcuteTtest(Double[] X, Double[] Y, double alpha, StringBuilder statusResult, int sampleIdx) { // boolean
		...
		statusResult.append("n A= " + n1 + ", " + "n B= " + n2 + ", " + "avg SAMPLE A= "
							+ StringManipulationTools.formatNumber(x_, "#.###") + ", " + "avg SAMPLE B="
							+ StringManipulationTools.formatNumber(y_, "#.###") + ", " + "variance A="
							+ StringManipulationTools.formatNumber(s2_1, "#.###") + ", " + "variance B="
							+ StringManipulationTools.formatNumber(s2_2, "#.###") + ", " + "|avg A - avg B|="
							+ StringManipulationTools.formatNumber(d_, "#.###") + ", " + "df=" + (n1 + n2 - 2) + "<br>");
		try {
			statusResult.append("<b>P [" + StringManipulationTools.formatNumber(ttest.homoscedasticTTest(xd, yd), "#.###")
								+ "] &lt; alpha [" + StringManipulationTools.formatNumber(alpha, "#.###") + "]? "
	...
	private boolean calcuteTestVonWelch(Double[] X, Double[] Y, double alpha, boolean useApache,
		...
		statusResult.append("n A= " + n1 + ", " + "n B= " + n2 + ", " + "avg SAMPLE A= "
							...
							+ StringManipulationTools.formatNumber(d_, "#.###") + ", " + "df=" + (n1 + n2 - 2) + "<br>");
		try {
			statusResult.append("<b>P [" + StringManipulationTools.formatNumber(ttest.tTest(xd, yd), "#.###") + "] &lt; alpha ["
								+ StringManipulationTools.formatNumber(alpha, "#.###") + "]? " + (res1 != null && res1 ? "YES" : "NO")
								+ " : Sample " + sampleIdx + "</b>");
		} catch (IllegalArgumentException e) {
			// empty
		} catch (MathException e) {
	...
	private boolean calcuteWilcoxonTest(Double[] X, Double[] Y, double alpha, StringBuilder statusResult, int sampleIdx) {
		...
		statusResult.append("TIE CORRECTION=" + special_sum + ", " + "z [" + StringManipulationTools.formatNumber(z_, "#.###")
							+ "]");
		double compare_z;
		try {
			compare_z = normalDistribution.inverseCumulativeProbability(1 - alpha / 2);
			statusResult.append(" &gt; z_alpha [" + StringManipulationTools.formatNumber(compare_z, "#.###") + "]");
							...
							if (!inRange)
								resultForNode = resultForNode
													...
													+ ("Time=" + timePoint + ", Series=" + serie + " (" + "PG="
																		+ StringManipulationTools.formatNumber(pg, "#.##") + ", " + "r="
																		+ StringManipulationTools.formatNumber(r, "#.##") + ", " + "s="
																		+ StringManipulationTools.formatNumber(stdDev, "#.##") + ", " + "n=" + n + ")");
							if (!inRange)
								allInRange = false;
						}
					}
					...
					if (Double.isNaN(x) || Double.isNaN(y))
						...
						if (rowDescription < maxROW)
							calculationHistory.append("<tr><td>" + dataset.getSeriesName(series) + "</td><td>" + item + " ("
												...
												+ "<td>" + (rankOrder ? StringManipulationTools.formatNumber(x_not_ranked, "#.###") + " -> " : "")
												+ StringManipulationTools.formatNumber(x, "#.###") + "</td>" + "<td>"
												+ (rankOrder ? StringManipulationTools.formatNumber(y_not_ranked, "#.###") + " -> " : "")
												+ StringManipulationTools.formatNumber(y, "#.###") + "</td></tr>\n");
						rowDescription++;
						sum_x += x;
						sum_y += y;
						sum_x_x += x * x;
	...
	private static void calcAndAddResult(int dataset2offset, double prob, CorrelationResult corrRes,
		...
		calculationHistory
							.append("</table><!-- optend --><br>" + "<table border=\"1\">" + "<tr>" + "<td><b>n</b></td>" + "<td><b>"
									...
									// +
									"<td>" + StringManipulationTools.formatNumber(t_or_z, "#.######") + "</td>" + "<td>" + (n - 2) + "</td>"
												+ "<td>" + StringManipulationTools.formatNumber(myp, "#.######") + "</td>" + warningText + "</tr>"
												+ "</table>");
