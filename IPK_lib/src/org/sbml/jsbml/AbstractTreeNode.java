/*
 * $Id: AbstractTreeNode.java,v 1.1 2012-11-07 14:43:34 klukas Exp $
 * $URL: https://jsbml.svn.sourceforge.net/svnroot/jsbml/trunk/core/src/org/sbml/jsbml/AbstractTreeNode.java $
 * ----------------------------------------------------------------------------
...
public abstract class AbstractTreeNode implements TreeNodeWithChangeSupport {
		...
		if (listOfListeners.size() > 0) {
			...
			if (-1 < changeType) {
				...
				if ((changeType == 0) && newValTreeNode) {
					...
					if ((changeType == 1) && oldValTreeNode) {
	...
	/**
	 * @param key
	 *           some user-defined key under which the given userObject can be found.
	 * @param userObject
