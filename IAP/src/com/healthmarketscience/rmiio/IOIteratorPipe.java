 ...
 * <p>
 * Note, although the Sink and Source are designed for use by separate threads, the objects themselves are not thread safe, so a given Sink/Source may not be
 * used by more than one thread without external synchronization.
 * 
...
public class IOIteratorPipe<DataType> {
	...
	/**
	 * Constructs an IOIteratorPipe with the <code>DEFAULT_QUEUE_SIZE</code> internal queue size.
	 */
	 ...
	 * 
	 * @param queueSize
	 *           the maximum number of objects which will be held by this
	...
	/**
	 * The Sink for this pipe. Data is added to the pipe via the <code>addNext</code> method. When all the data is added, the <code>setFinished</code> method
	 * should be called to indicate that all
	 ...
	 * IOIterator interface methods in the standard fashion. The <code>close</code> method should be called regardless of whether or not
	...
	public class Source extends AbstractCloseableIOIterator<DataType> {
		/**
		 * the next object to return from a call to <code>getNext</code>. If <code>null</code>, this object has not yet been initialized. If
