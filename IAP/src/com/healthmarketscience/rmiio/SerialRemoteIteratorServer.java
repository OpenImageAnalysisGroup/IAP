 ...
 * <p>
 * Note, the objects are written to the ObjectOutputStream using the {@link java.io.ObjectOutputStream#writeUnshared} method, and the
 * {@link java.io.ObjectOutputStream#reset} method is called periodically on the output stream. These measures are taken because memory can build up in the
 * ObjectOutputStream over time and a large data set can run the client and/or server out of memory. In general, the objects being iterated over most likely do
 * not have shared references, so nothing will be lost by this choice. However, if shared references are desired, the {@link #serializeObject} method can be
 * overriden by a custom subclass to change this behavior.
 * <p>
 * In the event that a RemoteIterator is being used to return low-latency, low-bandwidth update data to the client, the noDelay option can be enabled for the
 * underlying stream which will effectively disable buffering of data on the server side. This can be very useful for implementing remote progress monitors, for
 * example.
 * <p>
 * Note, since it is a common idiom for the local iterator to implement Closeable in order to close local resources, this implementation will automatically
 * close a Closeable local iterator after the underlying server is shutdown.
 * 
...
public class SerialRemoteIteratorServer<DataType>
	 ...
	 * 
	 * @param ostream
	 *           the output stream to which the object should be written
	 * @param obj
	 *           the object to write
	 */
