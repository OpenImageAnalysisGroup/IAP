 ...
 * <p>
 * This class is a plugin for the ImageJ/Fiji interface. It allows pairwise image registration combining the ideas of elastic registration based on B-spline
 ...
 * <p>
 * For more information visit the main site <A target="_blank" href="http://biocomp.cnb.csic.es/~iarganda/bUnwarpJ/">
...
{ /* begin class bUnwarpJ_ */
	...
	/** minimum image scale */
	private final int min_scale_image = 0;
	/** stopping threshold */
	 ...
	 * 
	 * @param commandLine
	 *           command to determine the action
	 */
	@Override
			...
			double stopThreshold)
	{
		...
		Transformation warp = computeTransformationBatch(targetImp, sourceImp, targetMskIP, sourceMskIP,
			...
			Param parameter)
	{
		...
		Transformation warp = computeTransformationBatch(targetImp, sourceImp, targetMskIP,
	 ...
	 * 
	 * @param args
	 *           arguments to decide the action
	 */
	...
	private static void alignImagesCommandLine(String args[])
	{
		...
		int min_scale_deformation = new Integer(args[5]).intValue();
		int max_scale_deformation = new Integer(args[6]).intValue();
		int max_subsamp_fact = new Integer(args[7]).intValue();
		double divWeight = new Double(args[8]).doubleValue();
		double curlWeight = new Double(args[9]).doubleValue();
		double imageWeight = new Double(args[10]).doubleValue();
		
		int accurate_mode = MainDialog.ACCURATE_MODE;
		
		double consistencyWeight = new Double(args[11]).doubleValue();
		...
		if (args.length == 17)
		{
			...
			{
				landmarkWeight = new Double(args[15]).doubleValue();
				fn_landmark = args[16];
			}
			else
				if (args[14].equals("-affine"))
				{
					fn_affine_1 = args[15];
					fn_affine_2 = args[16];
				}
	 ...
	 *           target image file name (with path)
	 * @param sourceImageName
	 *           source image file name (with path)
	 * @param transformationFileName
	 *           transformation file name (with path)
	 ...
	 * @param sourceImageName
	 *           source image file name (with path)
	 * @param targetTransfFileName
	 *           target transformation file name (with path)
	 * @param sourceTransfFileName
	 *           source transformation file name (with path)
	 */
	 ...
	 * @param sourceImageName
	 *           source image file name (with path)
	 * @param inputElasticTransfFileName
	 *           input transformation file name (with path)
	 * @param outputRawTransfFileName
	 *           output transformation file name (with path)
	 */
	 ...
	 * @param sourceImageName
	 *           source image file name (with path)
	 * @param inputElasticTransfFileName1
	 *           first input transformation file name (with path)
	 * @param inputElasticTransfFileName2
	 *           second input transformation file name (with path)
	 * @param outputRawTransfFileName
	 *           output transformation file name (with path)
	 */
	 ...
	 * @param sourceImageName
	 *           source image file name (with path)
	 * @param inputRawTransfFileName1
	 *           first input transformation file name (with path)
	 * @param inputRawTransfFileName2
	 *           second input transformation file name (with path)
	 * @param outputRawTransfFileName
	 *           output transformation file name (with path)
	 */
	 ...
	 * @param sourceImageName
	 *           source image file name (with path)
	 * @param inputRawTransfFileName
	 *           input raw transformation file name (with path)
	 * @param inputElasticTransfFileName
	 *           input elastic transformation file name (with path)
	 * @param outputRawTransfFileName
	 *           output transformation file name (with path)
	 */
	 ...
	 * @param inputElasticTransfFileName
	 *           input elastic transformation file name (with path)
	 * @param outputElasticTransfFileName
	 *           output elastic transformation file name (with path)
	 * @param sizeFactor
	 *           double size factor (between old and new image)
	 */
