...
{ /* begin class PointAction */
	 ...
	 * 
	 * @param secondaryImp
	 *           pointer to the secondary image
	 * @param secondaryPh
	 *           secondary point handler
	 */
			...
			final int y)
	{
		...
		if (mainImptype == ImagePlus.GRAY8 || mainImptype == ImagePlus.GRAY16) {
			...
			if (mainImptype == ImagePlus.GRAY32) {
				return (", value=" + Float.intBitsToFloat(v[0]));
			} else
				if (mainImptype == ImagePlus.COLOR_256) {
					return (", index=" + v[3] + ", value=" + v[0] + "," + v[1] + "," + v[2]);
				} else
					if (mainImptype == ImagePlus.COLOR_RGB) {
