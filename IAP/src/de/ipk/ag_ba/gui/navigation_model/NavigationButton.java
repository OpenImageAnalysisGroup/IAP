...
public class NavigationButton implements StyleAware {
	...
	boolean actionCausedDisable = false;
	
	public String getTitle(boolean forceProgressText) {
		// if (action != null && !action.mayRun()) {
		// enabled = false;
		// actionCausedDisable = true;
		// } else {
		// if (actionCausedDisable)
		// enabled = true;
		// }
		...
		if (!forceProgressText && !(isProcessing() || requestsTitleUpdates() != TitleUpdateMode.STATIC) || compTime < 1000) {
			...
			String cc2 = "*";
			if (ndots == 0)
				cc = (char) 0x25CB + ""; // 0x25AE + "";// "\\";
			if (ndots == 1)
				cc = (char) 0x25CF + ""; // 0x25AF + "";// "|";
			if (ndots == 0)
				cc2 = (char) 0x25CF + ""; // 0x25AF + "";// "\\";
			if (ndots == 1)
				cc2 = (char) 0x25CB + ""; // 0x25AE + "";// "|";
			// if (ndots == 2)
			// cc = (char) 0x25C6 + "";// "/";
			// if (ndots == 3)
			// cc = (char) 0x25F3 + "";// "-";
			...
			line2 = (s != null && !s.isEmpty() ? "<br>" : "") + line2;
			// if (line2.length() > 0)
			// line2 = "<br>&nbsp;" + line2 + "&nbsp;";
			//
			...
			if (dp < -1.01) {
				...
				if ((dots + dots2 + progress).length() > 0)
					return "<html><center><table border=0><tr><td>" + dots + "</td><td align='center'>" + title + progress + "</td><td>" + dots2 + "</td></tr></table>"
							+ s
							+ line2;
				else
					return title;
	...
	public static JComponent getNavigationButton(final ButtonDrawStyle style, final NavigationButton n,
		...
		final JButton n1 = new JButton() {
			...
			public void setText(String title) {
				String processedTitle = process(title);
				super.setText(processedTitle);
				if (!removeCalled)
				...
				if (target == PanelTarget.ACTION && n.getIconInactive() != null && n.getIconInactive().getImage() != null) {
					...
					if (target == PanelTarget.NAVIGATION) {
						if (SystemAnalysis.isRetina() && n.getNavigationImage() != null)
							...
							icon = peek ? GravistoService.peekIcon(IAPmain.class, n.getNavigationImage(), -imgS, imgS, false)
									: GravistoService.loadIcon(IAPmain.class, n.getNavigationImage(), -imgS, imgS, false);
						if (icon == null && n.getAction() != null) {
							icon = peek ? GravistoService.peekIcon(n.getAction().getClass(), n.getNavigationImage(), -imgS, imgS, true)
					...
					if (target != PanelTarget.NAVIGATION || icon == null) {
						...
						if (icon == null && n.getAction() != null) {
							icon = peek ? GravistoService.peekIcon(n.getAction().getClass(), n.getActionImage(), -imgS, imgS, true)
	...
	public boolean isRemoved() {
		boolean res = guiSetting == null || (!guiSetting.getNavigationPanel().getEntitySet(true).contains(this)
