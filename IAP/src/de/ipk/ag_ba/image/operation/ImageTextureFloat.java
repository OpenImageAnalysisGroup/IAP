 ...
 * @author pape
 */
public class ImageTextureFloat {
	 ...
	 */
	public ImageTextureFloat(Image inp) {
		float[] pix = (float[]) inp.getAsImagePlus().getProcessor().getPixels();
		img = new float[pix.length];
		int i = 0;
		for (float p : pix) {
	 ...
	 */
	public ImageTextureFloat(float[] inp, int width, int height) {
		img = inp;
		w = width;
		h = height;
	}
	...
	private double[][] getSymetricalFrameworkMatrix(boolean horizontal) {
		...
		for (int idx = 0; idx < w * h; idx++) {
			...
			if (horizontal) {
				if (idx + 1 < w * h) {
					x = (int) (img[idx] / 256d);
					y = (int) (img[idx + 1] / 256d);
				}
			} else {
				if (idx + w < w * h) {
					x = (int) (img[idx] / 256d);
					y = (int) (img[idx + w] / 256d);
				}
			}
