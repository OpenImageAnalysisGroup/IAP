...
/**
 * Method to extract Border Features from an image.
 * 
 * @author pape
 */
public class BorderAnalysis {
	 ...
	 **/
	public void getPeaksFromBorder(int minSizeOfPeak, int distBetweenPeaks, String filterKey) {
		...
		peaks = CurveAnalysis.findMaxima(data, minSizeOfPeak, distBetweenPeaks);
	...
	public void calcSUSAN(int radius, int geometricThresh) {
		...
		for (ArrayList<Integer> tempArray : borderLists) {
			...
			for (int idx = 0; idx < listSize; idx += 2) {
				...
				int[][] predefinedRegion = crop(img2d, w, h, xtemp - radius, xtemp + radius, ytemp - radius, ytemp + radius);
	...
	/**
	 * Method to detect if a region is split, that means it touched the border of an circle with two sides.
	 */
	private static boolean isSplit(int[][] region, ArrayList<PositionAndColor> regiona, int radius, boolean debug) throws InterruptedException {
