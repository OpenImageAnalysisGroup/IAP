...
public class TestPhytoTopAnalysis {
	...
	public void test() throws IOException, Exception {
		...
		PhytochamberTopImageProcessor test = new PhytochamberTopImageProcessor(options);
		// test.setValuesToStandard(scale);
		// test.clearBackground().getImages().getVis().print("Visible Test 1");
		
		FlexibleImageSet res = test.pipeline(input, SystemAnalysis.getNumberOfCPUs(), null, true).getImages();
