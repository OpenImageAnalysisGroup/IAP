...
//		printHashMap(clusterVerweis);
		...
		//die Endcluster werden beschrieben und es wird die Größe der Cluster bestimmt
		zweiterDurchlauf();
		...
		System.out.println("Clustergröße:");
...
//		|Pixel4|  x   |		 |	
		...
		{		//Ecke links oben und rechts oben
			...
			case 5:
					...
					Pixel4 = image[i][j-1];
						...
						//Pixel1 und Pixel2 müssten immer im selben Cluster liegen
						} else if(Pixel2 > zaehlerK-1 && Pixel3 > zaehlerK-1 && Pixel4 < zaehlerK){
							image[i][j] = Pixel3;
							//hashMapFuellen(i-1, j, i-1, j-1);
						
						} else {	
							//wenn alle dei Pixel zu einem Cluster gehören, dann wird der linke Cluster für den aktuellen Pixel genommen
							//es muss keine hashMap zuweisung stattfinden, da bereits alle drei Pixel um den aktuellen Pixel verknüpft wurden
							image[i][j] = Pixel4;
	...
	public void printList(ArrayList<Integer> liste)
	{
		for(int i = zaehlerK; i < liste.size(); i++)
			System.out.println("Cluster " + i + " enthält " + liste.get(i) + " Pixel");
	}
	...
	public void printHashMap(HashMap<Integer, ArrayList<Integer>> hashM){
		...
		if(!hashM.isEmpty())
			for( int clusterID : hashM.keySet() )
				System.out.println("Zu Cluster " + clusterID + " gehören folgenden Cluster: " + hashM.get(clusterID));
		else
			System.out.println("Es müssen keine Cluster fusioniert werden!");
	}
	...
	public void printArray(int[] zaehlerArray2) {
		...
		for( int arrayID = zaehlerK; arrayID < zaehlerArray2.length; arrayID++ )
			System.out.println("Cluster " + arrayID + " enthält " + zaehlerArray2[arrayID] + " Pixel");
