				...
				if (key.equals("image.subtitle")) {
					...
					if (key.equals("slice.label")) {
						...
						if (key.equals("window.contents")) {
							...
							if (key.equals("image.description")) {
								String description = "";
								...
								if (key.equals("image.filename")) {
									...
									if (key.equals("image.directory")) {
										...
										if (key.equals("selection.name") || key.equals("roi.name")) {
											ImagePlus imp = getImage();
											Roi roi = imp.getRoi();
											String name = roi != null ? roi.getName() : null;
											return name != null ? name : "";
										} else
											if (key.equals("font.name")) {
												resetImage();
												ImageProcessor ip = getProcessor();
												setFont(ip);
							...
							if (name.equals("setColor")) {
								...
								if (name.equals("add")) {
									...
									if (arg.indexOf("curve") != -1 || arg.indexOf("line") != -1)
										...
										if (arg.indexOf("box") != -1)
											what = Plot.BOX;
										else
											if (arg.indexOf("triangle") != -1)
												what = Plot.TRIANGLE;
											else
												if (arg.indexOf("cross") != -1)
													what = Plot.CROSS;
												else
													if (arg.indexOf("dot") != -1)
														what = Plot.DOT;
													else
														if (arg.indexOf("x") != -1)
															what = Plot.X;
		...
		try {
			...
			if (name.equals("addString")) {
				...
				if (name.equals("addNumber")) {
					...
					if (name.equals("addCheckbox")) {
						gd.addCheckbox(getFirstString(), getLastArg() == 1 ? true : false);
					} else
						if (name.equals("addCheckboxGroup")) {
							addCheckboxGroup(gd);
						} else
							if (name.equals("addMessage")) {
								gd.addMessage(getStringArg());
							} else
								if (name.equals("addHelp")) {
	...
	void setOption() {
		...
		if (arg1.equals("disablepopupmenu")) {
			...
			if (arg1.startsWith("show all")) {
				...
				if (arg1.equals("changes"))
					getImage().changes = state;
				else
					if (arg1.equals("debugmode"))
						IJ.debugMode = state;
					else
						if (arg1.equals("openusingplugins"))
							Opener.setOpenUsingPlugins(state);
						else
							if (arg1.equals("queuemacros"))
								pgm.queueCommands = state;
							else
								if (arg1.equals("disableundo"))
									Prefs.disableUndo = state;
								else
									if (arg1.startsWith("openashyper"))
										getImage().setOpenAsHyperStack(true);
									else
										if (arg1.startsWith("black"))
											Prefs.blackBackground = state;
										else
											if (arg1.startsWith("display lab"))
												Analyzer.setMeasurement(LABELS, state);
											else
												if (arg1.startsWith("limit to"))
					...
					if (name.equals("setFrame"))
						...
						if (name.equals("setDisplayMode"))
							...
							if (name.equals("getDisplayMode"))
								...
								if (name.equals("setActiveChannels"))
									setActiveChannels(imp, getStringArg());
								else
									if (name.equals("getActiveChannels"))
	...
	String doList() {
		...
		if (name.equals("get")) {
			...
			if (name.equals("getValue")) {
				...
				if (name.equals("set") || name.equals("add") || name.equals("put"))
						...
						if (name.equals("setList"))
							...
							if (name.equals("getList"))
								...
								if (name.equals("size") || name.equals("getSize")) {
									...
									if (name.equals("setMeasurements"))
										setMeasurements();
									else
										if (name.equals("setCommands"))
											setCommands();
										else
