...
public class ScaleBar implements PlugIn {
	...
	boolean showDialog(ImagePlus imp) {
		...
		if (roi != null) {
			...
			if (location.equals(locations[AT_SELECTION]))
		...
		if (roiX > 0 && roiWidth > 10)
			...
			if (barWidth == 0.0 || barWidth > 0.67 * imageWidth) {
	...
	boolean updateLocation() {
		...
		if (location.equals(locations[UPPER_RIGHT]))
			...
			if (location.equals(locations[LOWER_RIGHT]))
				y = height - height / fraction - barHeightInPixels - fontSize;
			else
				if (location.equals(locations[UPPER_LEFT])) {
					x = width / fraction;
					y = height / fraction;
				} else
					if (location.equals(locations[LOWER_LEFT])) {
						x = width / fraction;
						y = height - height / fraction - barHeightInPixels - fontSize;
					} else {
						if (roiX == -1)
					...
					if (color.equals(colors[4]))
						...
						if (color.equals(colors[5]))
							...
							if (color.equals(colors[6]))
								...
								if (color.equals(colors[7]))
									c = Color.blue;
								else
									if (color.equals(colors[8]))
										c = Color.yellow;
		return c;
	}
	...
	Color getBColor() {
		...
		if (bcolor.equals(bcolors[1]))
			...
			if (bcolor.equals(bcolors[3]))
				bc = Color.darkGray;
			else
				if (bcolor.equals(bcolors[4]))
					bc = Color.gray;
				else
					if (bcolor.equals(bcolors[5]))
						bc = Color.lightGray;
					else
						if (bcolor.equals(bcolors[6]))
							bc = Color.yellow;
						else
							if (bcolor.equals(bcolors[7]))
								bc = Color.blue;
							else
								if (bcolor.equals(bcolors[8]))
									bc = Color.green;
								else
									if (bcolor.equals(bcolors[9]))
										bc = Color.red;
		return bc;
	}
