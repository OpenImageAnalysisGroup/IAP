...
public class BackgroundSubtracter implements ExtendedPlugInFilter, DialogListener {
	 ...
	 * 
	 * @param pixels
	 *           Image data, will be modified by parabolic interpolation
	 *           where the parabola does not touch.
	 * @param start
	 *           Index of first pixel of the line in pixels array
	 * @param inc
	 *           Increment of index in pixels array
	 * @param length
	 *           Number of points the line consists of
	 * @param coeff2
	 *           2nd order coefficient of the polynomial describing the parabola,
	 *           must be positive (although a parabola with negative curvature is
	 *           actually used)
	 * @param cache
	 *           Work array, length at least <code>length</code>. Will usually remain
	 *           in the CPU cache and may therefore speed up the code.
	 * @param nextPoint
	 *           Work array. Will hold the index of the next point with sufficient local
	 *           curvature to get touched by the parabola.
	 * @param correctedEdges
	 *           Should be a 2-element array used for output or null.
	...
	static float[] lineSlideParabola(float[] pixels, int start, int inc, int length, float coeff2, float[] cache, int[] nextPoint, float[] correctedEdges) {
		...
		if (correctedEdges != null) {
			...
			correctedEdges[1] = value0 + (length - 1) * slope + coeff6 * (dx * dx * dx * dx * dx * dx - 1f) + coeff2 * (length - 1 - lastCorner)
					* (length - 1 - lastCorner);
	...
	void enlargeImage(FloatProcessor smallImage, FloatProcessor fp, int shrinkFactor) {
		...
		for (int x = 0; x < width; x++)
			// x-interpolation of the first smallImage line
		...
		for (int y = 0; y < height; y++) {
			if (ySmallLine0 < ySmallIndices[y]) {
				...
				for (int x = 0; x < width; x++)
					// x-interpolation of the new smallImage line -> line1
