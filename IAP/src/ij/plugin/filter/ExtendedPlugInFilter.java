 ...
 * <p>
 * - <code>showDialog(imp, command, pfr)</code>: The filter should display the dialog asking for parameters (if any) and do all operations needed to prepare for
 * processing the individual image(s) (E.g., slices of a stack). For preview, a separate thread may call <code>setNPasses(nPasses)</code> and
 * <code>run(ip)</code> while the dialog is displayed. The filter should return its flags.
 * <p>
 * - <code>setNPasses(nPasses)</code>: Informs the filter of the number of calls of <code>run(ip)</code> that will follow.
 * <p>
 * - <code>run(ip)</code>: Processing of the image(s). With the <code>CONVERT_TO_FLOAT</code> flag, this method will be called for each color channel of an RGB
 * image. With <code>DOES_STACKS</code>, it will be called for each slice of a stack.
 * <p>
 * - <code>setup("final", imp)</code>: called only if flag <code>FINAL_PROCESSING</code> has been specified.
 * <p>
...
public interface ExtendedPlugInFilter extends PlugInFilter {
	...
	/**
	 * This method is called after <code>setup(arg, imp)</code> unless the <code>DONE</code> flag has been set.
	 * 
	 * @param imp
	 *           The active image already passed in the <code>setup(arg, imp)</code> call. It will be null, however, if
	 ...
	 *           It can be passed to a GenericDialog by <code>addPreviewCheckbox</code> to enable preview by calling the <code>run(ip)</code> method of this
	 ...
	 *         of the flags specified in interfaces <code>PlugInFilter</code> and <code>ExtendedPlugInFilter</code>.
	 */
	 ...
	 * passes is one (or 3 for RGB images, if <code>CONVERT_TO_FLOAT</code> has been specified). When processing a stack, it is the number
