...
public class URLOpener implements PlugIn {
	...
	public void run(String urlOrName) {
		if (!urlOrName.equals("")) {
			if (urlOrName.endsWith("StartupMacros.txt"))
				...
				if (fi != null && fi.fileType == FileInfo.RGB48)
					...
					if (imp.getNChannels() > 1 && fi != null && fi.description != null && fi.description.indexOf("mode=") != -1) {
		...
		if (url.endsWith("/"))
			...
			if (url.endsWith(".html") || url.endsWith(".htm") || url.indexOf(".html#") > 0 || noExtension(url))
