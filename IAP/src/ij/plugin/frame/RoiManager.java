...
public class RoiManager extends PlugInFrame implements ActionListener, ItemListener, MouseListener, MouseWheelListener {
															...
															if (command.equals("Combine"))
																...
																if (command.equals("Split"))
																	...
																	if (command.equals("Add Particles"))
																		...
																		if (command.equals("Multi Measure"))
																			multiMeasure();
																		else
																			if (command.equals("Sort"))
																				sort();
																			else
																				if (command.equals("Specify..."))
																					specify();
																				else
																					if (command.equals("Remove Slice Info"))
																						removeSliceInfo();
																					else
																						if (command.equals("Help"))
																							...
																							if (command.equals("Options..."))
																								...
																								if (command.equals("\"Show All\" Color..."))
																									setShowAllColor();
	}
		...
		if (e.getStateChange() == ItemEvent.SELECTED && !ignoreInterrupts) {
			...
			if (!IJ.isMacintosh()) { // handle shift-click, ctrl-click (on Mac, OS takes care of this)
				...
				if (!IJ.shiftKeyDown() && !IJ.controlKeyDown()) { // simple click, deselect everything else
					...
					if (IJ.shiftKeyDown() && lastNonShiftClick >= 0 && lastNonShiftClick < list.getItemCount()) {
	...
	boolean isStandardName(String name) {
		...
		if (len >= 14 && name.charAt(4) == '-' && name.charAt(9) == '-')
			...
			if (len >= 17 && name.charAt(5) == '-' && name.charAt(11) == '-')
				...
				if (len >= 9 && name.charAt(4) == '-')
					isStandard = true;
				else
					if (len >= 11 && name.charAt(5) == '-')
						isStandard = true;
		return isStandard;
	}
	...
	public int getSliceNumber(String label) {
		...
		if (label.length() >= 14 && label.charAt(4) == '-' && label.charAt(9) == '-')
			...
			if (label.length() >= 17 && label.charAt(5) == '-' && label.charAt(11) == '-')
				slice = (int) Tools.parseDouble(label.substring(0, 5), -1);
			else
				if (label.length() >= 20 && label.charAt(6) == '-' && label.charAt(13) == '-')
	...
	boolean multiMeasure() {
		...
		new Analyzer(imp, measurements, rtMulti);
	...
	void sort() {
		...
		int index = 0;
		for (Enumeration<String> en = rois.keys(); en.hasMoreElements();)
	 ...
	 * "Sort" or "Multi Measure" command. Returns false if <code>cmd</code> is not one of these strings.
	 */
	public boolean runCommand(String cmd) {
		...
		if (cmd.equals("add"))
			...
			if (cmd.equals("add & draw"))
				addAndDraw(false);
			else
				if (cmd.equals("update"))
					update(true);
				else
					if (cmd.equals("update2"))
						update(false);
					else
												...
												if (cmd.equals("split"))
													...
													if (cmd.equals("sort"))
														...
														if (cmd.startsWith("multi"))
															...
															if (cmd.startsWith("show all")) {
																if (WindowManager.getCurrentImage() != null) {
																	showAll(SHOW_ALL);
																	showAllCheckbox.setState(true);
																}
															} else
																if (cmd.startsWith("show none")) {
																	if (WindowManager.getCurrentImage() != null) {
																		showAll(SHOW_NONE);
																		showAllCheckbox.setState(false);
																	}
																} else
																	if (cmd.equals("show all with labels")) {
																		labelsCheckbox.setState(true);
																		showAll(LABELS);
																		if (Interpreter.isBatchMode())
																			IJ.wait(250);
																	} else
																		if (cmd.equals("show all without labels")) {
																			labelsCheckbox.setState(false);
																			showAll(NO_LABELS);
																			if (Interpreter.isBatchMode())
																				IJ.wait(250);
																		} else
																			if (cmd.equals("deselect") || cmd.indexOf("all") != -1) {
																				if (IJ.isMacOSX())
