 ...
 * <p>
 * New update, correctly handling existing oval ROIs, the case that "Centered" is already selected when the plugin starts, and always restoring the original ROI
 * when the dialog is cancelled (JW, 2008/02/22) Enhancing the original plugin created by Jeffrey Kuhn, this one takes, in addition to width and height and the
 * option to have an oval ROI from the original program, x & y coordinates, slice number, and the option to have the x & y coordinates centered or in default
 * top left corner of ROI. The original creator is Jeffrey Kuhn, The University of Texas at Austin, jkuhn@ccwf.cc.utexas.edu
 * 
	...
	void showDialog() {
		...
		if (gd.wasCanceled()) {
			if (roi == null)
				...
				// *ALWAYS* restore initial ROI when cancelled
				imp.setRoi(roi);
		}
	}
