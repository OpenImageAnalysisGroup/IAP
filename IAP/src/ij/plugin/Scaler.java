...
public class Scaler implements PlugIn, TextListener, FocusListener {
	...
	boolean showDialog(ImageProcessor ip) {
		...
		if (macroOptions != null) {
			if (macroOptions.indexOf(" interpolate") != -1)
				...
				if (macroOptions.indexOf(" interpolation=") == -1)
	...
	public void textValueChanged(TextEvent e) {
		...
		if (source == xField && fieldWithFocus == xField) {
			...
			if (source == yField && fieldWithFocus == yField) {
				...
				if (source == zField && fieldWithFocus == zField) {
					...
					if (source == widthField && fieldWithFocus == widthField) {
						...
						if (source == depthField && fieldWithFocus == depthField) {
