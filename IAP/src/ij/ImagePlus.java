...
public class ImagePlus implements ImageObserver, Measurements {
	...
	public void show(String statusMessage) {
		...
		if ((img != null) && (width >= 0) && (height >= 0)) {
			...
			win.setTitle(win.getTitle() + ": " + statusMessage);
			if (roi != null)
				roi.setImage(this);
	...
	public void setStack(String title, ImageStack stack) {
		...
		if (stackSize == 1 && win instanceof StackWindow)
			...
			if (dimensionsChanged && !stackSizeChanged)
				win.updateImage(this);
			else
				if (stackSize > 1 && !(win instanceof StackWindow)) {
					...
					if (stackSize > 1 && (dimensionsChanged || invalidDimensions)) {
						if (isDisplayedHyperStack())
							setOpenAsHyperStack(true);
	...
	private final int[] pvalue = new int[4];
	 ...
	 * one slice. After adding or removing slices, call <code>setStack()</code> to update the image and
		...
		if (Roi.previousRoi != null) {
			...
			if (r.width <= width || r.height <= height || isSmaller(pRoi)) { // will it (mostly) fit in this image?
				...
				if (r.x >= width || r.y >= height || (r.x + r.width) <= 0 || (r.y + r.height) <= 0) // does it need to be moved?
					...
					if (r.width == width && r.height == height) // is it the same size as the image
						roi.setLocation(0, 0);
				draw();
			}
		}
	...
	protected void notifyListeners(int id) {
		synchronized (listeners) {
			for (int i = 0; i < listeners.size(); i++) {
				ImageListener listener = listeners.elementAt(i);
				switch (id) {
					case OPENED:
