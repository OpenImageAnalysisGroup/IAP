 ...
 * (i.e., union, intersection, exclusive or, and subtraction) provided by this class. These operators use the package <code>java.awt.geom</code> as a backend. <br>
...
public class ShapeRoi extends Roi {
	...
	/**
	 * Parsing a shape composed of linear segments less than this value will result in Roi objects of type {@link ij.gui.Roi#POLYLINE} and
	 ...
	 * respectively, in {@link ij.gui.Roi#FREELINE} and {@link ij.gui.Roi#FREEROI} (or {@link ij.gui.Roi#TRACED_ROI} if {@link #forceTrace} flag is <strong>
	...
	/**
	 * Flag which specifies how Roi objects will be constructed from closed (sub)paths having more than <code>MAXPOLY</code> and composed exclusively of line
	 * segments.
	 * If <strong><code>true</code></strong> then (sub)path will be parsed into a {@link ij.gui.Roi#TRACED_ROI}; else, into a {@link ij.gui.Roi#FREEROI}.
	 */
	 ...
	 * 
	 * @param r
	 *           An ij.gui.Roi object
	 * @param flatness
	 *           The flatness factor used in convertion of curve segments into line segments.
	 * @param maxerror
	 *           Error correction for calculating length of Bezeir curves.
	 * @param forceAngle
	 *           flag used in the conversion of Shape objects to Roi objects (see {@link #shapeToRois()}.
	 * @param forceTrace
	 *           flag for conversion of Shape objects to Roi objects (see {@link #shapeToRois()}.
	 * @param flatten
	 *           if <strong><code>true</code></strong> then the shape of this ROI will be flattened
	 *           (i.e., curve segments will be aproximated by line segments).
	 * @param maxPoly
	 *           Roi objects constructed from shapes composed of linear segments fewer than this
	 *           value will be of type {@link ij.gui.Roi#POLYLINE} or {@link ij.gui.Roi#POLYGON}; conversion of
	 *           shapes with linear segments more than this value will result in Roi objects of type {@link ij.gui.Roi#FREELINE} or {@link ij.gui.Roi#FREEROI}
	 ...
	 * and returns them in an array. <br>
	 ...
	 * <br>
	 * <br>
	 * Each subpath of a <code>java.awt.geom.GeneralPath</code> is converted following these rules:
	 * <table frame="border">
	...
	public Roi[] getRois() {
		...
		if (shape instanceof Rectangle2D.Double) {
			Roi r = new Roi((int) ((Rectangle2D.Double) shape).getX(), (int) ((Rectangle2D.Double) shape).getY(), (int) ((Rectangle2D.Double) shape).getWidth(),
					(int) ((Rectangle2D.Double) shape).getHeight());
			rois.addElement(r);
		} else
			if (shape instanceof Ellipse2D.Double) {
				Roi r = new OvalRoi((int) ((Ellipse2D.Double) shape).getX(), (int) ((Ellipse2D.Double) shape).getY(), (int) ((Ellipse2D.Double) shape).getWidth(),
						(int) ((Ellipse2D.Double) shape).getHeight());
				rois.addElement(r);
			} else
				if (shape instanceof Line2D.Double) {
					Roi r = new ij.gui.Line((int) ((Line2D.Double) shape).getX1(), (int) ((Line2D.Double) shape).getY1(), (int) ((Line2D.Double) shape).getX2(),
							(int) ((Line2D.Double) shape).getY2());
					rois.addElement(r);
				} else
	 ...
	 * 
	 * @param segments
	 *           The number of segments that compose the path
	 * @param linesOnly
	 *           Indicates wether the GeneralPath object is composed only of SEG_LINETO segments
	 * @param curvesOnly
	 *           Indicates wether the GeneralPath object is composed only of SEG_CUBICTO and SEG_QUADTO segments
	 * @param closed
	 *           Indicates a closed GeneralPath
	 * @see #shapeToRois()
	 * @return a type flag
	 */
	 ...
	 * &quot;Jens Gravesen. Adaptive subdivision and the length and energy of B&eacute;zier curves. Computational Geometry <strong>8:</strong><em>13-31</em>
	 * (1997)&quot;
	 ...
	 * 
	 * @param c
	 *           A cubic B&eacute;zier curve to be divided
	 ...
	 * 
	 * @param c
	 *           A quadratic B&eacute;zier curve to be divided
	 ...
	 * 
	 * @param c
	 *           Array of coordinates in double precision with a <strong>fixed</strong> structure:<br>
	 ...
	 * 
	 * @param pIter
	 *           the PathIterator to be parsed.
	 * @param params
	 *           an array with one elemet that will hold the calculated length of path;
	 * @param segments
	 *           a Vector of Integer objects that will hold the types of the segments composing the shape's path
	 * @param rois
	 *           a Vector that will hold ij.gui.Roi objects constructed from elements of this path
	 *           (see @link #shapeToRois()} for details;
	 * @param handles
	 *           a Vector of Point2D.Double objects representing vertices (segment joinings) and
