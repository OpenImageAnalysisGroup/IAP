...
public class ImageCanvas extends Canvas implements MouseListener, MouseMotionListener, Cloneable {
			...
			default: // selection tool
				if (id == Toolbar.SPARE1 || id >= Toolbar.SPARE2) {
					...
					if (roi != null && roi.getState() != Roi.CONSTRUCTING && roi.isHandle(sx, sy) >= 0)
						...
						if (Prefs.usePointerCursor || (roi != null && roi.getState() != Roi.CONSTRUCTING && roi.contains(ox, oy)))
							setCursor(defaultCursor);
						else
							setCursor(crosshairCursor);
		}
	...
	protected Dimension canEnlarge(int newWidth, int newHeight) {
		...
		if (fitsHorizontally && fitsVertically)
			...
			if (fitsVertically && newHeight < dstWidth)
				return new Dimension(dstWidth, newHeight);
			else
				if (fitsHorizontally && newWidth < dstHeight)
	...
	public void zoomOut(int x, int y) {
		...
		imp.getWindow().getInitialMagnification();
