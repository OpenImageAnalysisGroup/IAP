...
public class MathUtils3D {
	...
	public void test() {
		...
		assertEquals("angle 0,-4 = 270°", Math.PI + Math.PI / 2, getAngle(0, -4),
				epsilon);
		assertEquals("angle -2,4 = 116.565°", 116.565 / 180 * Math.PI, getAngle(
				-2, 4), epsilon);
		assertEquals("angle -4,-4 = 225°", Math.PI + Math.PI / 4,
				getAngle(-4, -4), epsilon);
		assertEquals("angle 2,-4 = 296.565°", 296.565 / 180 * Math.PI, getAngle(
				2, -4), epsilon);
	}
}
