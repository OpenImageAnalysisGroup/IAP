...
public class DataFileHandling {
	...
	public synchronized RemoteInputStream downloadFile(OracleConnection conn, String md5,
			boolean returnPreview) throws RemoteException {
		String sqlCommand = "SELECT length(data) as len, data, databfile, filesize "+
			"FROM imageFile "+
			"WHERE md5=?";
		if (returnPreview)
			sqlCommand = "SELECT length(previewImage) as len, previewImage "+
			"FROM imageFile "+
			"WHERE md5=?";
