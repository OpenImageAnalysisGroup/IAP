...
public class ImageConverter {
	...
	try {
		...
		GravistoService.showImage(scalingJAI(new URL("http://www.spiegel.de/images/image-150632-panoV9free-hldq.jpg")), "Scaling JAI Faktor 2");
	...
	protected static ImagePlus convertBItoIJ(BufferedImage img){
		ImagePlus jImage = new ImagePlus("JImage", img);
		return jImage;
	}
	
	protected static ImagePlus convert1AtoIJ(int w, int h, int [] img){
		ImagePlus jImage = new ImagePlus("JImage", convert1AtoBI(w, h, img));
		return jImage;
	}
	
	protected static ImagePlus convert2AtoIJ(int [][] img){
	...
	protected static int[] convertBIto1A(BufferedImage img) {
	...
	protected static int[] convert2Ato1A(int [][] img) {
	...
	protected static int[] convertIJto1A(ImagePlus img){
	...
	protected static int[][] convertBIto2A(BufferedImage img){
		int[][] image = convert1Ato2A(img.getWidth(), img.getHeight(), convertBIto1A(img));
		return image;
	}
	
	protected static int[][] convert1Ato2A(int w, int h, int [] img){
	...
	protected static int[][] convertIJto2A(ImagePlus img){
		BufferedImage image = ImageConverter.convertIJtoBI(img);
	...
	protected static BufferedImage convertPItoBI(PlanarImage plImage1) {
	...
	protected static BufferedImage convertIJtoBI(ImagePlus jImage1) {
		
		BufferedImage fBufferedImage = jImage1.getProcessor().getBufferedImage();
		return fBufferedImage;
	}
	
	protected static BufferedImage convert1AtoBI(int width, int height, int [] img){
	...
	protected static BufferedImage convert2AtoBI(int [][] img) {
	...
	protected static void printImage(int[][] image) {
		printImage(image, "Image");
	}

	protected static void printImage(int[][] image, String text) {
		printImage(image, "Image", 0, image.length, 0, image[0].length);
	}
	
	protected static void printImage(int[][] image, String text, int xPos, int xLength, int yPos, int yLength) {
...
//		scalingOwn.doZoom(2, Scaling.BILINEAR);
	...
	private static BufferedImage scalingAWT(BufferedImage img){
		...
		Image scalingAWT_temp = img.getScaledInstance(2*img.getWidth(), 2*img.getHeight(), Image.SCALE_AREA_AVERAGING);
