...
public class GenericDialog extends Dialog implements ActionListener, TextListener,
	...
	protected Vector<Component> numberField, stringField, checkbox, choice, slider;
	protected TextArea textArea1, textArea2;
	protected Vector<Comparable> defaultValues, defaultText;
	...
	private Hashtable<Component, String> labels;
	...
	public void addNumericField(String label, double defaultValue, int digits, int columns, String units) {
		...
		add(theLabel);
		if (numberField == null) {
			numberField = new Vector<Component>(5);
			defaultValues = new Vector<Comparable>(5);
			defaultText = new Vector<Comparable>(5);
		}
		if (IJ.isWindows())
			columns -= 2;
		if (columns < 1)
	...
	private void saveLabel(Component component, String label) {
		if (labels == null)
			labels = new Hashtable<Component, String>();
		if (label.length() > 0) {
			if (label.charAt(0) == ' ')
				label = label.trim();
	...
	public void addStringField(String label, String defaultText, int columns) {
		...
		if (stringField == null) {
			stringField = new Vector<Component>(4);
	...
	private void addCheckbox(String label, boolean defaultValue, boolean isPreview) {
		...
		if (label2.indexOf('_') != -1)
			label2 = label2.replace('_', ' ');
		if (checkbox == null) {
			checkbox = new Vector<Component>(4);
	...
	public void addCheckboxGroup(int rows, int columns, String[] labels, boolean[] defaultValues, String[] headings) {
		...
		if (checkbox == null)
			checkbox = new Vector<Component>(12);
		if (headings != null) {
	...
	public String getNextText() {
		...
		if (textAreaIndex == 0 && textArea1 != null) {
			...
			if (textAreaIndex == 1 && textArea2 != null) {
				textArea2.selectAll();
	...
	public void keyPressed(KeyEvent e) {
		...
		if (keyCode == KeyEvent.VK_ENTER && textArea1 == null) {
			...
			if (keyCode == KeyEvent.VK_ESCAPE) {
				...
				if (keyCode == KeyEvent.VK_W && (e.getModifiers() & Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()) != 0) {
					wasCanceled = true;
					dispose();
				}
	}
