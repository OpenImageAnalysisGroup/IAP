...
public class Toolbar extends Canvas implements MouseListener, MouseMotionListener, ItemListener, ActionListener {
	...
	public boolean setTool(String name) {
		...
		if (name.indexOf("round") != -1) {
			...
			if (name.indexOf("rect") != -1) {
				...
				if (name.indexOf("ellip") != -1 || name.indexOf("oval") != -1) {
					brushEnabled = false;
					setTool(OVAL);
				} else
					if (name.indexOf("brush") != -1) {
						...
						if (name.indexOf("polygon") != -1)
							...
							if (name.indexOf("polyline") != -1)
								...
								if (name.indexOf("freeline") != -1)
									setTool(FREELINE);
								else
									if (name.indexOf("line") != -1) {
										arrowMode = false;
										setTool(LINE);
									} else
														...
														if (name.indexOf("wand") != -1)
															...
															if (name.indexOf("text") != -1)
																...
																if (name.indexOf("hand") != -1)
																	...
																	if (name.indexOf("zoom") != -1)
																		setTool(MAGNIFIER);
																	else
																		if (name.indexOf("dropper") != -1 || name.indexOf("color") != -1)
																			setTool(DROPPER);
																		else
																			if (name.indexOf("angle") != -1)
																				setTool(ANGLE);
																			else
																				ok = false;
		return ok;
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (item == rectItem || item == roundRectItem) {
			...
			if (item == ovalItem || item == brushItem) {
				...
				if (item == pointItem || item == multiPointItem) {
	...
	void showBrushDialog() {
		...
		if (roi != null && roi.getType() == Roi.OVAL && brushEnabled)
			img.killRoi();
		Prefs.set(BRUSH_SIZE, brushSize);
	}
