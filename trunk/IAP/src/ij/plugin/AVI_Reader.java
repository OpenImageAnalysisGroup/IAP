...
public class AVI_Reader extends VirtualStack implements PlugIn {
	...
	public void run(String arg) {
		...
		try {
			makeStack(path, firstFrameNumber, lastFrameNumber,
							isVirtual, convertToGray, flipVertical);
		} catch (Exception e) {
			IJ.showMessage("AVI Reader", exceptionMessage(e));
			return;
		}
	...
	class raInputStream extends InputStream {
		...
		private void doFixMJPG(byte[] b, int len) {
			...
			if (readShort(b, 0) != 0xffd8 || len < 6)
				return; // not a start of JPEG-like data
			int offset = 2;
			while (true) {
				int code = readShort(b, offset); // read segment type
				if (code == 0xffda || code == 0xffd9)
					return; // start of image data or end
				if (code == 0xffe2) {
				...
				if (offset > len - 4 || segmentLength < 0)
					return;// pointed out of file
			}
		}
