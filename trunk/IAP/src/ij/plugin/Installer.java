...
public class Installer implements PlugIn {
	...
	void installPlugin() {
		...
		if (menuStr.equals(menus[0]))
			...
			if (menuStr.equals(menus[1]))
				menu = Menus.PLUGINS_MENU;
			else
				if (menuStr.equals(menus[2]))
					menu = Menus.IMPORT_MENU;
				else
					if (menuStr.equals(menus[3]))
						menu = Menus.SAVE_AS_MENU;
					else
						if (menuStr.equals(menus[4]))
							menu = Menus.FILTERS_MENU;
						else
							if (menuStr.equals(menus[5]))
								menu = Menus.TOOLS_MENU;
							else
								if (menuStr.equals(menus[6]))
