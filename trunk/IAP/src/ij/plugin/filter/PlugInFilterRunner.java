...
public class PlugInFilterRunner implements Runnable, DialogListener {
	...
	Hashtable<Thread, Integer> sliceForThread = new Hashtable<Thread, Integer>(); // here the stack slice currently processed is stored.
	 ...
	 * 
	 * @param theFilter
	 *           The PlugInFilter to be run
	 * @param command
	 *           The command that has caused running the PlugInFilter
	 * @param arg
	 *           The argument specified for this PlugInFilter in IJ_Props.txt or in the
	 *           plugins.config file of a .jar archive conatining a collection of plugins. <code>arg</code> may be a string of length zero.
	 */
	 ...
	 * 
	 * @param firstSlice
	 *           Slice number of the first slice to be processed
	 * @param endSlice
	 *           Slice number of the last slice to be processed
	 */
	 ...
	 * 
	 * @param ip
	 *           The image data that should be processed
	 * @param fp
	 *           A Floatprocessor as a target for conversion to Float. May be null.
	 * @param snapshotDone
	 *           Whether a snapshot of ip has been be taken previously. if
	...
	public void run() {
		...
		try {
			if (thread == previewThread)
				...
				if (slicesForThread != null && slicesForThread.containsKey(thread)) {
