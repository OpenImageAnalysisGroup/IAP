...
public class ControlPanel implements PlugIn {
	...
	private Hashtable<String, TreePanel> panels = new Hashtable<String, TreePanel>();
	private Vector<String> visiblePanels = new Vector<String>();
	private Vector<String> expandedNodes = new Vector<String>();
	...
	Vector<?> menus = new Vector<Object>();
	Vector<?> allMenus = new Vector<Object>();
	String[] installableMenuLabels = { "About Plugins", "Filters", "Import", "Plugins", "Save As", "Shortcuts", "Tools", "Utilities" };
	Hashtable<?, ?> commands = new Hashtable<Object, Object>();
	Hashtable<?, ?> menuCommands = new Hashtable<Object, Object>();
	String[] pluginsArray;
	Hashtable<String, String> treeCommands = new Hashtable<String, String>();
	int argLength = 0;
	 ...
	 * 
	 * @param arg
	 *           String (optional): a semi-colon - separated list of one or more tokens:<br>
	 *           <dl>
	 *           <dt>"imagej menus"</dt>
	 *           <dd>creates a tree with all of ImageJ's menu structure, that means it replicates in tree form the ImageJ's menu bar; this includes any jar
	 *           plugin and user plugins</dd>
	 *           <dt>"user plugins"</dt>
	 *           <dd>creates a tree with loose user plugins (not "jar plugins")</dd>
	 *           <dt>"imagej commands"</dt>
	 *           <dd>creates a tree with all ImageJ's commands</dd>
	 *           <dt>"about"</dt>
	 *           <dd>will not create a tree panel; instead, it will show a brief help message</dd>
	 *           </dl>
	 ...
	 * delegates the construction of the TreeNode to the <code>doRoot(String)</code> method. Else, this method delegates to the <code>doRootFromMenus()</code>
	 * method.
	 * 
	 * @param arg
	 *           See {@see run(String)} comments for what arguments are allowed.
	 ...
	 * 
	 * @param arg
	 *           See {@see run(String)} comments for a full description.
	 ...
	 * their labels correspond to keys in the hashtable returned by <code>ij.Menus.getCommands()</code> except for the "Reload Plugins" menu item, for which a
	 * local action command string is assigned
	 * to avoid clashes with the action fired from ImageJ Plugins->Utilties->Reload Plugins
	 * menu item.<br>
	 * <strong>Note: </strong> this method bypasses the tree buildup based on the {@link populateNode(Hashtable,DefaultMutableTreeNode)} method.
	 * 
	 * @param menu
	 *           The Menu instance to be searched recursively for menu items
	 * @param node
	 *           The DefaultMutableTreeNode corresponding to the <code>Menu menu</code> argument.
	 */
				...
				if (mItem instanceof MenuItem) {
					if (!(label.equals("-"))) {
						...
						if (treeCommands == null)
							treeCommands = new Hashtable<String, String>();
	 ...
	 * 
	 * @param collection
	 *           Hashtable with `keys' (java.lang.String) representing a tree path which
	 ...
	 * 
	 * @param items
	 *           String array where each element is the source of a tree path (see {@see buildTreePath(String, String, DefaultMutableTreeNode)} and {@see
	 *           buildTreePath(String,String,String,DefaultMutableTreeNode)}.
	 * @param labels
	 *           String array where each element is the label of the root of the tree path
	 * @param node
	 *           The TreeNode to be populated
	 */
	private void populateNode
	 ...
	 * 
	 * @param source
	 *           String to be parsed in a tree path; must be composed of tokens delimited by "/"
	 * @param label
	 *           The label (String) of the leaf node for this path. If <code><strong>null</strong></code> then the leafe nod will be constructed from the last
	 *           token.
	 * @param command
	 *           The command string of the action event fired upon clicking the leaf node.
	 *           If <code><strong>null</strong></code>, then the last token will be taken as action command.
	 * @param topNode
	 *           The DefaulMutableTreeNode to which this path will be added
	 */
	private void buildTreePath
			(
					String source,
	...
	Hashtable<String, TreePanel> getPanels() {
		return panels;
	}
	
	Hashtable<String, String> getTreeCommands() {
		return treeCommands;
	}
	 ...
	 * <ol>
	 * <li>a space-separated list of integers, indicating panel frame geometry <strong>in the following fixed order:</strong> x coordinate, y coordinate , width,
	 * height</li>
	 * <li>or the word "expand" which indicates that the path represented by the keyword is an expanded branch</li>
	 * </ol>
	 */
	void loadProperties() {
	...
	void saveProperties() {
		if (propertiesChanged) {
			pcpProperties.clear();
			for (Enumeration<String> e = visiblePanels.elements(); e.hasMoreElements();) {
				String s = (String) e.nextElement();
				TreePanel p = (TreePanel) panels.get(s);
				if (p != null)
					recordGeometry(p);
			}
			for (Enumeration<String> e = expandedNodes.elements(); e.hasMoreElements();) {
	...
	void closeAll(boolean die) {
		...
		for (Enumeration<TreePanel> e = panels.elements(); e.hasMoreElements();) {
...
class TreePanel implements
	...
	{
			0x21, 0xff000000, 0xff303030, 0xffaaaaaa, 0xffffffff, 0xff3c3c3c, 0xff252525, 0xffb6b6b6, 0xff585858, 0xffc3c3c3, 0xff222222, 0xff2b2b2b, 0xff2e2e2e,
			0xffa0a0a0,
			0xff808080
	};
		...
		// for(Enumeration e = root.children(); e.hasMoreElements();)
		{
			...
			if (!node.isLeaf() && node != root) {
				...
				if (pcp.hasExpandedStateProperty(npS) && !pcp.hasPanelShowingProperty(npS)) {
					...
					if ((pcp.hasExpandedStateProperty(npS) || pTree.isExpanded(newPath)) && pcp.hasPanelShowingProperty(npS)) {
	...
	public void treeExpanded(TreeExpansionEvent ev) {
		...
		if (pcp.hasPanelShowingProperty(path)) {
			Hashtable<String, TreePanel> panels = pcp.getPanels();
