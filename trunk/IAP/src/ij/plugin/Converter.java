		...
		try {
			if (stack != null) {
				...
				if (stack.isRGB() && item.equals("RGB Color")) {
					...
					if (stack.isHSB() && item.equals("RGB Color")) {
						...
						if (item.equals("8-bit"))
							...
							if (item.equals("16-bit"))
								new StackConverter(imp).convertToGray16();
							else
								if (item.equals("32-bit"))
									new StackConverter(imp).convertToGray32();
								else
									if (item.equals("RGB Color"))
										new StackConverter(imp).convertToRGB();
									else
										if (item.equals("RGB Stack"))
											new StackConverter(imp).convertToRGBHyperstack();
										else
											if (item.equals("HSB Stack"))
