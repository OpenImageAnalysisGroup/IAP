...
public class Menus {
	...
	private Hashtable<?, ?> demoImagesTable = new Hashtable<Object, Object>();
	...
	private static Vector<String> macroFiles; // Macro files in plugins folder with "_" in their name
	...
	void installJarPlugin(String jar, String s) {
		...
		if (s.startsWith("Plugins>")) {
			...
			if (s.startsWith("\"") || s.startsWith("Plugins")) {
		...
		if (result == null) {
			...
			if (offset < 0) {
				...
				if (readFromProps)
					...
					if (parentName.startsWith("Plugins") && menuSeparators != null)
	...
	void setupPluginsAndMacrosPaths() {
		...
		if (homeDir.endsWith("plugins"))
			...
			if (pluginsDir == null)
				...
				if (pluginsDir.equals("user.home")) {
	...
	public static Hashtable<?, ?> getMacroShortcuts() {
		if (macroShortcuts == null)
			macroShortcuts = new Hashtable<Object, Object>();
	...
	static boolean validShortcut(String shortcut) {
		...
		if (shortcut.equals(""))
			...
			if (len == 1)
				...
				if (shortcut.startsWith("F") && (len == 2 || len == 3))
					return true;
				else
					return false;
	}
