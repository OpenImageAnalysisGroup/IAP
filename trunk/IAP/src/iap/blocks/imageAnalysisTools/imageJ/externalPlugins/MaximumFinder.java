...
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {
	...
	boolean showResultsTable = false;
	
	public ResultsTable getRt() {
		return rt;
	}
	
	public void setShowResultsTable(boolean b) {
		this.showResultsTable = b;
	}
	 ...
	 * 
	 * @param arg
	 *           Not used by this plugin-filter
	 * @param imp
	 ...
	 * 
	 * @param ip
	 *           The image where maxima (or minima) should be found
	 */
	@Override
			...
			for (int y = 0, i = 0; y < outIp.getHeight(); y++) { // delete everything outside roi
				for (int x = 0; x < outIp.getWidth(); x++, i++) {
					if (x < roi.x || x >= roi.x + roi.width || y < roi.y || y >= roi.y + roi.height)
						...
						if (mask != null && (mask[x - roi.x + roi.width * (y - roi.y)] == 0))
							outPixels[i] = (byte) 0;
				}
			}
		}
	 ...
	 *           sorting may be inaccurate, sequence may be reversed for maxima having values
					...
					for (int d = 0; d < 8; d++) { // analyze all neighbors (in 8 directions) at the same level
						...
						if ((isInner || isWithin(x, y, d)) && (types[offset2] & LISTED) == 0) {
							if ((types[offset2] & MAX_AREA) != 0 || (((types[offset2] & ELIMINATED) != 0) && (pixels[offset2] & 255) >= loLevel)) {
								...
								if ((pixels[offset2] & 255) >= loLevel && (types[offset2] & ELIMINATED) == 0) {
									pList[listLen] = offset2;
		...
		for (int d = 0; d < 8; d++) { // walk around the point and note every no-line->line transition
			...
			if (isInner || isWithin(x, y, d)) {
				...
				if ((d & 1) == 0)
					...
					if (!isSet) // diagonal directions may separate two lines,
